<template>
  <div class="profile-page" :class="{ 'page-shake': userInfo.status === 0 }">
    <!-- 蜂巢装饰背景 -->
    <div class="hex-bg" />
            ...
            <div v-else-if="(activeTab === 'following' || activeTab === 'followers') && users.length > 0" class="users-list">
              <TransitionGroup name="card-anim">
                <UserCard
                  ...
                  :user="user"
                  :show-follow="isSelf"
                  :follow-back="activeTab === 'followers' && isSelf"
                  class="stagger-item"
...
@keyframes stamp-appear {
  ...
  40% { opacity: 1; transform: scale(0.88) rotate(-8deg); }
