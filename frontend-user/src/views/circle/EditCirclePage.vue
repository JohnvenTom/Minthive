    ...
    <main class="edit-main">
      ...
      <section class="form-section" :style="{ animationDelay: '240ms' }">
        <div class="section-label-row">
          ...
          <span class="section-hint">圆形头像（可选）</span>
        </div>
      ...
      <section class="form-section" :style="{ animationDelay: '320ms' }">
        <div class="section-label-row">
          ...
          <span class="section-hint">建议尺寸 1600×900（可选）</span>
        </div>
...
import { getCircleDetail, updateCircle, getCategories } from '@/api/circle'
...
/**
 * 当前编辑的圈子ID
 * @description 从路由参数中获取，用于加载和更新圈子数据
 */
...
/**
 * 原始圈子数据快照
 * @description 用于对比判断哪些字段发生了变化，避免发送未修改的字段
 */
...
/**
 * 原始圈子类型值
 * @description 后端返回的 type 可能是字符串 "public"/"private" 或数字 0/1，统一转为字符串存储
 */
...
// ---------- 本地缓存的文件（提交时才上传） ----------
/** 头像裁剪后的 File 对象（为空表示未重新上传） */
const avatarFile = ref<File | null>(null)
/** 封面裁剪后的 File 对象（为空表示未重新上传） */
...
/**
 * 判断表单是否可以提交
 * @returns {boolean} 名称填写且无校验错误时返回 true
 * @description 编辑模式放宽校验规则：仅要求名称必填(2-20字)，其他字段均为可选
 */
...
/**
 * 移除已选择的头像
 * @description 编辑模式下移除头像后，提交时会将该字段置空（清除原有头像）
 */
...
/**
 * 移除已选择的封面
 * @description 编辑模式下移除封面后，提交时会将该字段置空（清除原有封面）
 */
...
async function confirmCrop(): Promise<void> {
  ...
  try {
    ...
    console.error('[EditCircle] 裁剪失败:', err)
    showToast('裁剪失败，请重试')
  } finally {
 ...
 * @description 编辑模式放宽校验规则：仅校验名称(2-20字)，简介、头像、封面均为可选
 */
function validateForm(): boolean {
  ...
  // 分类校验：可选，仅在选择了自定义但未填写内容时报错
  if (showCustomInput.value && !customCategoryText.value.trim() && !form.categoryId) {
    errors.category = '请输入自定义分类名称或选择预设分类'
    valid = false
  } else {
    errors.category = ''
  }
 ...
 * @description 先校验表单，再上传新图片到 MinIO，最后调用更新圈子 API（只发送变化的字段）
 */
async function onSubmit(): Promise<void> {
  ...
  try {
    ...
    // 2. 比较并添加分类变化
    if (form.categoryId !== originalData.categoryId) {
      if (form.categoryId) {
        updateData.categoryId = form.categoryId
      } else if (form.categoryName.trim() && form.categoryName.trim() !== originalData.categoryName) {
        updateData.categoryName = form.categoryName.trim()
      }
    }

    // 3. 比较并添加简介变化
    if (form.intro.trim() !== originalData.intro) {
      updateData.intro = form.intro.trim()
    }
    ...
    if (bannerFile.value) {
      ...
      const bannerUrl = typeof res.data === 'string' ? res.data : res.data?.url || ''
    ...
    console.error('[EditCircle] 更新圈子失败:', err)
    showToast('保存失败，请重试')
  } finally {
 ...
 * 从后端获取当前圈子详情数据并回填到表单
 * @returns {Promise<void>}
 * @description 页面挂载时调用 getCircleDetail API 获取圈子数据，初始化表单和原始数据快照
 * @throws 当圈子ID无效或API调用失败时抛出错误
 */
async function loadCircleDetail(): Promise<void> {
  try {
    ...
    // 后端返回的 type 是字符串 "public"/"private"，兼容数字格式
    originalType.value = typeof data.type === 'string' ? data.type : (data.type === 0 ? 'public' : 'private')

    // 缓存原始数据用于后续变更检测
    originalData.name = form.name
    originalData.categoryId = form.categoryId
    originalData.categoryName = data.categoryName || ''
    originalData.intro = form.intro
    originalData.avatar = form.avatar
    originalData.banner = form.banner
    originalData.type = originalType.value
  } catch (err) {
    console.error('[EditCircle] 加载圈子详情失败:', err)
    showToast('加载圈子信息失败')
    router.back()
  }
}
...
async function fetchCategories(): Promise<void> {
  ...
  try {
    ...
    console.warn('[EditCircle] 获取分类失败，使用默认列表:', err)
    categories.value = [
...
/**
 * 返回上一页
 * @description 如果表单有修改则弹出确认框，否则直接返回
 */
function goBack(): void {
  // 检查是否有实际修改（与原始数据对比）
  const hasChanges =
    form.name.trim() !== originalData.name ||
    form.categoryId !== originalData.categoryId ||
    form.categoryName.trim() !== originalData.categoryName ||
    form.intro.trim() !== originalData.intro ||
    !!avatarFile.value ||
    !!bannerFile.value
